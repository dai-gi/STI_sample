...
class BusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bus" do
    assert_difference("Bus.count") do
      post buses_url, params: { bus: { name: @bus.name, number_of_seater: @bus.number_of_seater, size: @bus.size } }
    end
  ...
  test "should update bus" do
    patch bus_url(@bus), params: { bus: { name: @bus.name, number_of_seater: @bus.number_of_seater, size: @bus.size } }
